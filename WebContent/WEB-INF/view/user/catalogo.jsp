...
<main>
    ...
    <%-- FUNZIONE HELPER PER FILTRARE E MOSTRARE I PRODOTTI DI UNA CATEGORIA --%>
    <%! 
        // Metodo helper per stampare i box dei prodotti in base alle parole chiave presenti nel nome/descrizione
        void renderCategoria(HttpServletRequest request, JspWriter out, List<Prodotto> prodotti, String keyword) throws java.io.IOException {
            ...
            for (Prodotto p : prodotti) {
                ...
                String desc = p.getDescrizione().toLowerCase();
                String kw = keyword.toLowerCase();
                
                // Controllo di appartenenza alla categoria
                ...
                if (kw.equals("regali") || kw.equals("peluche")) {
                    ...
                    appartiene = nome.contains("terrarium") || nome.contains("marimo");
                }

                if (appartiene) {
                    // Gestione dinamica del percorso immagine (se ha già la sottocartella o meno)
                    String imgPath = p.getImmagine();
                    String srcTarget = "";
                    if (imgPath != null && (imgPath.contains("/") || imgPath.contains("\\"))) {
                        srcTarget = request.getContextPath() + "/images/" + imgPath;
                    } else {
                        srcTarget = request.getContextPath() + "/images/" + imgPath;
                    }
